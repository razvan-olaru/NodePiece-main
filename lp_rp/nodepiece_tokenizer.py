...
class NodePiece_Tokenizer:
    ...
    def tokenize_kg(self):
        ...
        path = Path(filename)
        if path.is_file():
            anchors, non_anchors, vocab, graph = pickle.load(open(path, "rb"))
        ...
        pickle.dump((top_entities, non_core_entities, vocab, graph), open(filename, "wb"))
